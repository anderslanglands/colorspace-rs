...
pub fn rgb_to_spd_smits_refl(rgb: RGBf32) -> SPD {
    ...
    if rgb.r <= rgb.g && rgb.r <= rgb.b {
        r += rgb.r * RGB2SPD_REFL_WHITE.clone();
        if rgb.g <= rgb.b {
            r += (rgb.g - rgb.r) * RGB2SPD_REFL_CYAN.clone();
            r += (rgb.b - rgb.g) * RGB2SPD_REFL_BLUE.clone();
        } else {
            r += (rgb.b - rgb.r) * RGB2SPD_REFL_CYAN.clone();
            r += (rgb.g - rgb.b) * RGB2SPD_REFL_GREEN.clone();
        }
    } else if rgb.g <= rgb.r && rgb.g <= rgb.b {
        r += rgb.g * RGB2SPD_REFL_WHITE.clone();
        if rgb.r <= rgb.b {
            r += (rgb.r - rgb.g) * RGB2SPD_REFL_MAGENTA.clone();
            r += (rgb.b - rgb.r) * RGB2SPD_REFL_BLUE.clone();
        } else {
            r += (rgb.b - rgb.g) * RGB2SPD_REFL_MAGENTA.clone();
            r += (rgb.r - rgb.b) * RGB2SPD_REFL_RED.clone();
        }
    } else {
        r += rgb.b * RGB2SPD_REFL_WHITE.clone();
        if rgb.r <= rgb.g {
            r += (rgb.r - rgb.b) * RGB2SPD_REFL_YELLOW.clone();
            r += (rgb.g - rgb.r) * RGB2SPD_REFL_GREEN.clone();
        } else {
            r += (rgb.g - rgb.b) * RGB2SPD_REFL_YELLOW.clone();
            r += (rgb.r - rgb.g) * RGB2SPD_REFL_RED.clone();
        }
    }
...
// pbrt source code is Copyright(c) 1998-2016
// Matt Pharr, Greg Humphreys, and Wenzel Jakob.
/// Convert RGB colour `rgb` to a reflectance value for the
/// given wavelength `lambda` using Smits's method:
/// http://www.cs.utah.edu/~bes/papers/color/
pub fn rgb_to_spd_smits_refl_single(rgb: RGBf32, lambda: f32) -> f32 {
    let mut r = 0.0;

    if rgb.r <= rgb.g && rgb.r <= rgb.b {
        r += rgb.r * RGB2SPD_REFL_WHITE.value_at(lambda);
        if rgb.g <= rgb.b {
            r += (rgb.g - rgb.r) * RGB2SPD_REFL_CYAN.value_at(lambda);
            r += (rgb.b - rgb.g) * RGB2SPD_REFL_BLUE.value_at(lambda);
        } else {
            r += (rgb.b - rgb.r) * RGB2SPD_REFL_CYAN.value_at(lambda);
            r += (rgb.g - rgb.b) * RGB2SPD_REFL_GREEN.value_at(lambda);
        }
    } else if rgb.g <= rgb.r && rgb.g <= rgb.b {
        r += rgb.g * RGB2SPD_REFL_WHITE.value_at(lambda);
        if rgb.r <= rgb.b {
            r += (rgb.r - rgb.g) * RGB2SPD_REFL_MAGENTA.value_at(lambda);
            r += (rgb.b - rgb.r) * RGB2SPD_REFL_BLUE.value_at(lambda);
        } else {
            r += (rgb.b - rgb.g) * RGB2SPD_REFL_MAGENTA.value_at(lambda);
            r += (rgb.r - rgb.b) * RGB2SPD_REFL_RED.value_at(lambda);
        }
    } else {
        r += rgb.b * RGB2SPD_REFL_WHITE.value_at(lambda);
        if rgb.r <= rgb.g {
            r += (rgb.r - rgb.b) * RGB2SPD_REFL_YELLOW.value_at(lambda);
            r += (rgb.g - rgb.r) * RGB2SPD_REFL_GREEN.value_at(lambda);
        } else {
            r += (rgb.g - rgb.b) * RGB2SPD_REFL_YELLOW.value_at(lambda);
            r += (rgb.r - rgb.g) * RGB2SPD_REFL_RED.value_at(lambda);
        }
    }
...
pub fn rgb_to_spd_smits_illum(rgb: RGBf32) -> SPD {
    ...
    if rgb.r <= rgb.g && rgb.r <= rgb.b {
        r += rgb.r * RGB2SPD_ILLUM_WHITE.clone();
        if rgb.g <= rgb.b {
            r += (rgb.g - rgb.r) * RGB2SPD_ILLUM_CYAN.clone();
            r += (rgb.b - rgb.g) * RGB2SPD_ILLUM_BLUE.clone();
        } else {
            r += (rgb.b - rgb.r) * RGB2SPD_ILLUM_CYAN.clone();
            r += (rgb.g - rgb.b) * RGB2SPD_ILLUM_GREEN.clone();
        }
    } else if rgb.g <= rgb.r && rgb.g <= rgb.b {
        r += rgb.g * RGB2SPD_ILLUM_WHITE.clone();
        if rgb.r <= rgb.b {
            r += (rgb.r - rgb.g) * RGB2SPD_ILLUM_MAGENTA.clone();
            r += (rgb.b - rgb.r) * RGB2SPD_ILLUM_BLUE.clone();
        } else {
            r += (rgb.b - rgb.g) * RGB2SPD_ILLUM_MAGENTA.clone();
            r += (rgb.r - rgb.b) * RGB2SPD_ILLUM_RED.clone();
        }
    } else {
        r += rgb.b * RGB2SPD_ILLUM_WHITE.clone();
        if rgb.r <= rgb.g {
            r += (rgb.r - rgb.b) * RGB2SPD_ILLUM_YELLOW.clone();
            r += (rgb.g - rgb.r) * RGB2SPD_ILLUM_GREEN.clone();
        } else {
            r += (rgb.g - rgb.b) * RGB2SPD_ILLUM_YELLOW.clone();
            r += (rgb.r - rgb.g) * RGB2SPD_ILLUM_RED.clone();
        }
    }
...
/// Convert RGB colour `rgb` to an illuminant `SPD` using Mallett and Yuksel's
/// method:
/// https://geometrian.com/data/research/spectral-primaries/EGSR2019_SpectralPrimaryDecomposition.pdf
pub fn rgb_to_spd_mallett(rgb: RGBf32) -> SPD {
    rgb.r * MALLETT_BASIS_R.clone()
        + rgb.g * MALLETT_BASIS_G.clone()
        + rgb.b * MALLETT_BASIS_B.clone()
}


pub fn rgb_to_spd_mallett_single(rgb: RGBf32, lambda: f32) -> f32 {
    rgb.r * MALLETT_BASIS_R.value_at(lambda)
        + rgb.g * MALLETT_BASIS_G.value_at(lambda)
        + rgb.b * MALLETT_BASIS_B.value_at(lambda)
}


#[test]
fn test_rgb_to_spd_smits() {
    use crate::prelude::*;
    ...
    for name in names {
        ...
        let srgb_u8 = RGBu8::from(oetf::srgb(rgb));

        for y in (row * patch_h)..(row * patch_h + 50) {
            for x in (col * patch_w)..(col * patch_w + 50) {
                let i = (y * im_w + x) * 3;
                pixels_orig[i + 0] = srgb_u8.r;
                pixels_orig[i + 1] = srgb_u8.g;
                pixels_orig[i + 2] = srgb_u8.b;
            }
        }
        ...
        for y in row * patch_h..row * patch_h + 50 {
            for x in col * patch_w..col * patch_w + 50 {
                let i = (y * im_w + x) * 3;
                pixels_ups[i + 0] = srgb_u8.r;
                pixels_ups[i + 1] = srgb_u8.g;
...
lazy_static! {
    pub static ref RGB2SPD_REFL_WHITE: SPD = SPD::from_wavelength_and_value(
        &[
    ...
    pub static ref RGB2SPD_REFL_CYAN: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_REFL_MAGENTA: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_REFL_YELLOW: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_REFL_RED: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_REFL_GREEN: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_REFL_BLUE: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_ILLUM_WHITE: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_ILLUM_CYAN: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_ILLUM_MAGENTA: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_ILLUM_YELLOW: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_ILLUM_RED: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_ILLUM_GREEN: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref RGB2SPD_ILLUM_BLUE: SPD = SPD::from_wavelength_and_value(
        &[
            380.000000, 390.967743, 401.935486, 412.903229, 423.870972,
            434.838715, 445.806458, 456.774200, 467.741943, 478.709686,
            489.677429, 500.645172, 511.612915, 522.580627, 533.548340,
    ...
    pub static ref MALLETT_BASIS_R: SPD = SPD::from_range_and_values(
        380.0,
        780.0,
        &[
            0.327457413827055,
    ...
    pub static ref MALLETT_BASIS_G: SPD = SPD::from_range_and_values(
        380.0,
        780.0,
        &[
            0.331861713085874,
    ...
    pub static ref MALLETT_BASIS_B: SPD = SPD::from_range_and_values(
        380.0,
        780.0,
        &[
            0.340680791548052,
